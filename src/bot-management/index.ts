...
export function hasBotManagementException(
  ...
  if (!botManagement) {
    console.warn(
      "[mpp-proxy] Bot Management Filtering configured but cf.botManagement not available. " +
  ...
  if (botScore === undefined || botScore === null) {
    console.warn(
      "[mpp-proxy] Bot Management data available but score is missing. " +
  ...
  const exceptDetectionIds = config.except_detection_ids ?? [];
  if (exceptDetectionIds.length > 0) {
    const isExcepted = detectionIds.some((id) =>
      exceptDetectionIds.includes(id),
    );
    if (isExcepted) {
